...
namespace Bam.Net.Data.Tests
{
	...
	public partial class Cart: Bam.Net.Data.Dao
	{
		public Cart():base()
		{
		...
		public Cart(Database db, DataRow data)
			: base(db, data)
		{
			this.SetKeyColumnName();
			this.SetChildren();
		...
		private void SetChildren()
		{
			if(_database != null)
			{
				this.ChildCollections.Add("CartItem_CartId", new CartItemCollection(Database.GetQuery<CartItemColumns, CartItem>((c) => c.CartId == GetLongValue("Id")), this, "CartId"));				
			}						
		}
	...
	public CartItemCollection CartItemsByCartId
	{
		...
		public static CartCollection LoadAll(Database database = null)
		{
			...
			var results = new CartCollection(db, sql.GetDataTable(db));
			results.Database = db;
			return results;
		}
		...
		/// </summary>
		[Bam.Net.Exclude]
		public static async Task BatchAll(int batchSize, Action<IEnumerable<Cart>> batchProcessor, Database database = null)
		{
			await Task.Run(async ()=>
			{
				CartColumns columns = new CartColumns();
				var orderBy = Bam.Net.Data.Order.By<CartColumns>(c => c.KeyColumn, Bam.Net.Data.SortOrder.Ascending);
		...
		/// </summary>			 
		[Bam.Net.Exclude]
		public static async Task BatchQuery(int batchSize, QueryFilter filter, Action<IEnumerable<Cart>> batchProcessor, Database database = null)
		{
		...
		/// </summary>	
		[Bam.Net.Exclude]
		public static async Task BatchQuery(int batchSize, WhereDelegate<CartColumns> where, Action<IEnumerable<Cart>> batchProcessor, Database database = null)
		{
			await Task.Run(async ()=>
			{
				CartColumns columns = new CartColumns();
				...
				while(results.Count > 0)
				{
					...
					long topId = results.Select(d => d.Property<long>(columns.KeyColumn.ToString())).ToArray().Largest();
					results = Top(batchSize, (CartColumns)where(columns) && columns.KeyColumn > topId, orderBy, database);
				}
			});			
		}
		...
		/// </summary>			 
		[Bam.Net.Exclude]
		public static async Task BatchQuery<ColType>(int batchSize, QueryFilter filter, Action<IEnumerable<Cart>> batchProcessor, Bam.Net.Data.OrderBy<CartColumns> orderBy, Database database = null)
		{
		...
		/// </summary>	
		[Bam.Net.Exclude]
		public static async Task BatchQuery<ColType>(int batchSize, WhereDelegate<CartColumns> where, Action<IEnumerable<Cart>> batchProcessor, Bam.Net.Data.OrderBy<CartColumns> orderBy, Database database = null)
		{
			await Task.Run(async ()=>
			{
				CartColumns columns = new CartColumns();
				...
				while(results.Count > 0)
				{
					...
					results = Top(batchSize, (CartColumns)where(columns) && orderBy.Column > top, orderBy, database);
				}
			});			
		}
		...
		public static Cart GetByUuid(string uuid, Database database = null)
		{
			return OneWhere(c => Bam.Net.Data.Query.Where("Uuid") == uuid, database);
		}

		public static Cart GetByCuid(string cuid, Database database = null)
		{
		...
		[Bam.Net.Exclude]
		public static CartCollection Top(int count, QueryFilter where, Database database)
		{
			return Top(count, where, null, database);
		}
		/// <summary>
