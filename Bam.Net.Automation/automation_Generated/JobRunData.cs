...
namespace Bam.Net.Automation.Data
{
	...
	public JobData JobDataOfJobDataId
	{
		...
		public static async Task BatchAll(int batchSize, Func<JobRunDataCollection, Task> batchProcessor, Database database = null)
		{
			await Task.Run(async ()=>
			{
				JobRunDataColumns columns = new JobRunDataColumns();
				var orderBy = Order.By<JobRunDataColumns>(c => c.KeyColumn, SortOrder.Ascending);
		...
		public static async Task BatchQuery(int batchSize, QueryFilter filter, Func<JobRunDataCollection, Task> batchProcessor, Database database = null)
		{
			await BatchQuery(batchSize, (c) => filter, batchProcessor, database);			
		}

		public static async Task BatchQuery(int batchSize, WhereDelegate<JobRunDataColumns> where, Func<JobRunDataCollection, Task> batchProcessor, Database database = null)
		{
			await Task.Run(async ()=>
			{
				JobRunDataColumns columns = new JobRunDataColumns();
