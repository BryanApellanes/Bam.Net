...
namespace Bam.Net.Automation.Data
{
	...
	[Serializable]
	[Bam.Net.Data.Table("JobData", "Automation")]
	public partial class JobData: Dao
	{
		public JobData():base()
		{
		...
		public JobData(Database db, DataRow data)
			: base(db, data)
		{
			this.SetKeyColumnName();
			this.SetChildren();
	...
	public JobRunDataCollection JobRunDatasByJobDataId
	{
		get
		{
			...
			if(!this.ChildCollections.ContainsKey("JobRunData_JobDataId"))
			{
				SetChildren();
			}

			var c = (JobRunDataCollection)this.ChildCollections["JobRunData_JobDataId"];
			if(!c.Loaded)
			{
				c.Load(Database);
			}
		...
		public override IQueryFilter GetUniqueFilter()
		{
			...
			{
				var colFilter = new JobDataColumns();
		...
		public static JobDataCollection LoadAll(Database database = null)
		{
			...
			var results = new JobDataCollection(sql.GetDataTable(db));
			results.Database = db;
			return results;
		}

		public static async Task BatchAll(int batchSize, Func<JobDataCollection, Task> batchProcessor, Database database = null)
		{
		...
		public static async Task BatchQuery(int batchSize, QueryFilter filter, Func<JobDataCollection, Task> batchProcessor, Database database = null)
		{
			await BatchQuery(batchSize, (c) => filter, batchProcessor, database);			
		}

		public static async Task BatchQuery(int batchSize, WhereDelegate<JobDataColumns> where, Func<JobDataCollection, Task> batchProcessor, Database database = null)
		{
			await Task.Run(async ()=>
			{
				JobDataColumns columns = new JobDataColumns();
				...
				while(results.Count > 0)
				{
					await batchProcessor(results);
					long topId = results.Select(d => d.Property<long>(columns.KeyColumn.ToString())).ToArray().Largest();
					results = Top(batchSize, (JobDataColumns)where(columns) && columns.KeyColumn > topId, orderBy, database);
				}
			});			
		}
		...
		public static JobData GetByUuid(string uuid, Database database = null)
		{
			return OneWhere(c => Bam.Net.Data.Query.Where("Uuid") == uuid, database);
		}

		public static JobData GetByCuid(string cuid, Database database = null)
		{
		...
		public static JobDataCollection Where(Func<JobDataColumns, QueryFilter<JobDataColumns>> where, OrderBy<JobDataColumns> orderBy = null, Database database = null)
		{
			...
			return new JobDataCollection(database.GetQuery<JobDataColumns, JobData>(where, orderBy), true);
		}
		...
		{		
			database = database ?? Db.For<JobData>();
			var results = new JobDataCollection(database, database.GetQuery<JobDataColumns, JobData>(where), true);
			return results;
		}
		...
		{		
			database = database ?? Db.For<JobData>();
			var results = new JobDataCollection(database, database.GetQuery<JobDataColumns, JobData>(where, orderBy), true);
			return results;
		}
		...
		/// <param name="database"></param>
		public static JobDataCollection Where(QiQuery where, Database database = null)
		{
			var results = new JobDataCollection(database, Select<JobDataColumns>.From<JobData>().Where(where, database));
			return results;
		}
		...
		/// <param name="where"></param>
		/// <param name="database"></param>
		public static JobData OneWhere(QiQuery where, Database database = null)
		{
		...
		public static JobDataCollection Top(int count, WhereDelegate<JobDataColumns> where, OrderBy<JobDataColumns> orderBy, Database database = null)
		{
			...
			query.Top<JobData>(count);
			query.Where(filter);

			if(orderBy != null)
			{
				query.OrderBy<JobDataColumns>(orderBy);
			}

			query.Execute(db);
			var results = query.Results.As<JobDataCollection>(0);
			results.Database = db;
			return results;
		}
		...
		/// </param>
		/// <param name="db"></param>
		public static JobDataCollection Top(int count, QiQuery where, Database database = null)
		{
			Database db = database ?? Db.For<JobData>();
			QuerySet query = GetQuerySet(db);
			query.Top<JobData>(count);
		...
		private static JobData OneOrThrow(JobDataCollection c)
		{
			if(c.Count == 1)
			{
				return c[0];
