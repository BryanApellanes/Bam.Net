...
namespace Bam.Net.Automation.ContinuousIntegration.Data
{
	...
	[Serializable]
	[Bam.Net.Data.Table("BuildResult", "ContinuousIntegration")]
	public partial class BuildResult: Bam.Net.Data.Dao
	{
		...
		public BuildResult(DataRow data)
			: base(data)
		{
			this.SetKeyColumnName();
			this.SetChildren();
		...
		public BuildResult(Database db)
			: base(db)
		{
			this.SetKeyColumnName();
			this.SetChildren();
		...
		public BuildResult(Database db, DataRow data)
			: base(db, data)
		{
			this.SetKeyColumnName();
			this.SetChildren();
	...
	public BuildJob BuildJobOfBuildJobId
	{
		get
		{
			...
			{
				_buildJobOfBuildJobId = Bam.Net.Automation.ContinuousIntegration.Data.BuildJob.OneWhere(c => c.KeyColumn == this.BuildJobId, this.Database);
			}
		...
		/// </summary>
		[Bam.Net.Exclude]
		public static async Task BatchAll(int batchSize, Action<IEnumerable<BuildResult>> batchProcessor, Database database = null)
		{
			await System.Threading.Tasks.Task.Run(async ()=>
			{
				BuildResultColumns columns = new BuildResultColumns();
				var orderBy = Bam.Net.Data.Order.By<BuildResultColumns>(c => c.KeyColumn, Bam.Net.Data.SortOrder.Ascending);
		...
		/// </summary>			 
		[Bam.Net.Exclude]
		public static async Task BatchQuery(int batchSize, QueryFilter filter, Action<IEnumerable<BuildResult>> batchProcessor, Database database = null)
		{
		...
		/// </summary>	
		[Bam.Net.Exclude]
		public static async Task BatchQuery(int batchSize, WhereDelegate<BuildResultColumns> where, Action<IEnumerable<BuildResult>> batchProcessor, Database database = null)
		{
			await System.Threading.Tasks.Task.Run(async ()=>
			{
				BuildResultColumns columns = new BuildResultColumns();
				...
				while(results.Count > 0)
				{
					...
					long topId = results.Select(d => d.Property<long>(columns.KeyColumn.ToString())).ToArray().Largest();
					results = Top(batchSize, (BuildResultColumns)where(columns) && columns.KeyColumn > topId, orderBy, database);
				}
			});			
		}
		...
		/// </summary>			 
		[Bam.Net.Exclude]
		public static async Task BatchQuery<ColType>(int batchSize, QueryFilter filter, Action<IEnumerable<BuildResult>> batchProcessor, Bam.Net.Data.OrderBy<BuildResultColumns> orderBy, Database database = null)
		{
		...
		/// </summary>	
		[Bam.Net.Exclude]
		public static async Task BatchQuery<ColType>(int batchSize, WhereDelegate<BuildResultColumns> where, Action<IEnumerable<BuildResult>> batchProcessor, Bam.Net.Data.OrderBy<BuildResultColumns> orderBy, Database database = null)
		{
			await System.Threading.Tasks.Task.Run(async ()=>
			{
				BuildResultColumns columns = new BuildResultColumns();
				...
				while(results.Count > 0)
				{
					...
					results = Top(batchSize, (BuildResultColumns)where(columns) && orderBy.Column > top, orderBy, database);
				}
			});			
		}

		public static BuildResult GetById(uint id, Database database = null)
		{
		...
		public static BuildResult GetById(ulong id, Database database = null)
		{
			return OneWhere(c => c.KeyColumn == id, database);
		}

		public static BuildResult GetByUuid(string uuid, Database database = null)
		{
			return OneWhere(c => Bam.Net.Data.Query.Where("Uuid") == uuid, database);
		}

		public static BuildResult GetByCuid(string cuid, Database database = null)
		{
		...
		[Bam.Net.Exclude]
		public static BuildResultCollection Top(int count, QueryFilter where, Database database)
		{
			return Top(count, where, null, database);
		}
		/// <summary>
		...
		[Bam.Net.Exclude]
		public static BuildResultCollection Top(int count, QueryFilter where, string orderBy = null, SortOrder sortOrder = SortOrder.Ascending, Database database = null)
		{
			Database db = database ?? Db.For<BuildResult>();
			QuerySet query = GetQuerySet(db);
			query.Top<BuildResult>(count);
