...
namespace Bam.Net.Analytics
{
	...
	public partial class LoadTimer: Bam.Net.Data.Dao
	{
	...
	public Timer TimerOfTimerId
	{
		...
		/// </summary>			 
		[Bam.Net.Exclude]
		public static async Task BatchQuery(int batchSize, QueryFilter filter, Action<IEnumerable<LoadTimer>> batchProcessor, Database database = null)
		{
		...
		/// </summary>			 
		[Bam.Net.Exclude]
		public static async Task BatchQuery<ColType>(int batchSize, QueryFilter filter, Action<IEnumerable<LoadTimer>> batchProcessor, Bam.Net.Data.OrderBy<LoadTimerColumns> orderBy, Database database = null)
		{
		...
		public static async Task BatchQuery<ColType>(int batchSize, WhereDelegate<LoadTimerColumns> where, Action<IEnumerable<LoadTimer>> batchProcessor, Bam.Net.Data.OrderBy<LoadTimerColumns> orderBy, Database database = null)
		{
			await System.Threading.Tasks.Task.Run(async ()=>
			{
				...
				while(results.Count > 0)
				{
					...
					results = Top(batchSize, (LoadTimerColumns)where(columns) && orderBy.Column > top, orderBy, database);
				}
			});			
		}
		...
		[Bam.Net.Exclude]
		public static LoadTimerCollection Top(int count, QueryFilter where, Database database)
		{
			return Top(count, where, null, database);
		}
		/// <summary>
