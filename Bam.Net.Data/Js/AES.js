...
if (FileExt !== undefined && FileExt !== null) {
    var validateKeySize = function (keySize) {
        if (!(keySize == 128 || keySize == 192 || keySize == 256)) {
            throw "Invalid keysize specified for AES encryption, must be : 128, 192 or 256.";
        }
    }

    FileExt.AES = {};
    FileExt.AES.encrypt = function (plaintext, password, keySize) {
        validateKeySize(keySize);
        return AesCtr.encrypt(plaintext, password, keySize);
    }

    FileExt.AES.decrypt = function (ciphertext, password, keySize) {
        validateKeySize(keySize);
        return AesCtr.decrypt(ciphertext, password, keySize);
    }

    FileExt.Utf8 = Utf8;
    FileExt.Base64 = Base64;
}
