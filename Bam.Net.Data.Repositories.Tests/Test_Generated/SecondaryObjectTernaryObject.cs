...
namespace Bam.Net.Data.Repositories.Tests
{
	...
	[Serializable]
	[Bam.Net.Data.Table("SecondaryObjectTernaryObject", "RepoTests")]
	public partial class SecondaryObjectTernaryObject: Bam.Net.Data.Dao
	{
		...
		public SecondaryObjectTernaryObject(DataRow data)
			: base(data)
		{
			this.SetKeyColumnName();
			this.SetChildren();
		...
		public SecondaryObjectTernaryObject(Database db)
			: base(db)
		{
			this.SetKeyColumnName();
			this.SetChildren();
		...
		public SecondaryObjectTernaryObject(Database db, DataRow data)
			: base(db, data)
		{
			this.SetKeyColumnName();
			this.SetChildren();
	...
	public SecondaryObject SecondaryObjectOfSecondaryObjectId
	{
		get
		{
			...
			{
				_secondaryObjectOfSecondaryObjectId = Bam.Net.Data.Repositories.Tests.SecondaryObject.OneWhere(c => c.KeyColumn == this.SecondaryObjectId, this.Database);
			}
	...
	public TernaryObject TernaryObjectOfTernaryObjectId
	{
		get
		{
			...
			{
				_ternaryObjectOfTernaryObjectId = Bam.Net.Data.Repositories.Tests.TernaryObject.OneWhere(c => c.KeyColumn == this.TernaryObjectId, this.Database);
			}
		...
		public static async Task BatchQuery(int batchSize, WhereDelegate<SecondaryObjectTernaryObjectColumns> where, Action<IEnumerable<SecondaryObjectTernaryObject>> batchProcessor, Database database = null)
		{
			await System.Threading.Tasks.Task.Run(async ()=>
			{
				...
				while(results.Count > 0)
				{
					...
					long topId = results.Select(d => d.Property<long>(columns.KeyColumn.ToString())).ToArray().Largest();
					results = Top(batchSize, (SecondaryObjectTernaryObjectColumns)where(columns) && columns.KeyColumn > topId, orderBy, database);
				}
			});			
		}
		...
		/// </summary>			 
		[Bam.Net.Exclude]
		public static async Task BatchQuery<ColType>(int batchSize, QueryFilter filter, Action<IEnumerable<SecondaryObjectTernaryObject>> batchProcessor, Bam.Net.Data.OrderBy<SecondaryObjectTernaryObjectColumns> orderBy, Database database = null)
		{
		...
		/// </summary>	
		[Bam.Net.Exclude]
		public static async Task BatchQuery<ColType>(int batchSize, WhereDelegate<SecondaryObjectTernaryObjectColumns> where, Action<IEnumerable<SecondaryObjectTernaryObject>> batchProcessor, Bam.Net.Data.OrderBy<SecondaryObjectTernaryObjectColumns> orderBy, Database database = null)
		{
			await System.Threading.Tasks.Task.Run(async ()=>
			{
				SecondaryObjectTernaryObjectColumns columns = new SecondaryObjectTernaryObjectColumns();
				...
				while(results.Count > 0)
				{
					...
					results = Top(batchSize, (SecondaryObjectTernaryObjectColumns)where(columns) && orderBy.Column > top, orderBy, database);
				}
			});			
		}

		public static SecondaryObjectTernaryObject GetById(uint id, Database database = null)
		{
		...
		public static SecondaryObjectTernaryObject GetById(ulong id, Database database = null)
		{
			return OneWhere(c => c.KeyColumn == id, database);
		}
		...
		public static SecondaryObjectTernaryObject GetByCuid(string cuid, Database database = null)
		{
		...
		/// <summary>
		/// Return the count of SecondaryObjectTernaryObjects
		/// </summary>
		/// <param name="database">
		/// Which database to query or null to use the default
		/// </param>
