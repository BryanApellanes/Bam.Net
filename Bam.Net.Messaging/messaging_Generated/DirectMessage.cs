...
namespace Bam.Net.Messaging.Data
{
	...
	public partial class DirectMessage: Bam.Net.Data.Dao
	{
		...
		public DirectMessage(DataRow data)
			: base(data)
		{
			this.SetKeyColumnName();
			this.SetChildren();
		...
		public DirectMessage(Database db)
			: base(db)
		{
			this.SetKeyColumnName();
			this.SetChildren();
		...
		public DirectMessage(Database db, DataRow data)
			: base(db, data)
		{
			this.SetKeyColumnName();
			this.SetChildren();
	...
	public Message MessageOfMessageId
	{
		get
		{
			...
			{
				_messageOfMessageId = Bam.Net.Messaging.Data.Message.OneWhere(c => c.KeyColumn == this.MessageId, this.Database);
			}
	...
	public EmailMessageCollection EmailMessagesByDirectMessageId
	{
		...
		/// </summary>			 
		[Bam.Net.Exclude]
		public static async Task BatchQuery<ColType>(int batchSize, QueryFilter filter, Action<IEnumerable<DirectMessage>> batchProcessor, Bam.Net.Data.OrderBy<DirectMessageColumns> orderBy, Database database = null)
		{
		...
		public static async Task BatchQuery<ColType>(int batchSize, WhereDelegate<DirectMessageColumns> where, Action<IEnumerable<DirectMessage>> batchProcessor, Bam.Net.Data.OrderBy<DirectMessageColumns> orderBy, Database database = null)
		{
			await System.Threading.Tasks.Task.Run(async ()=>
			{
				...
				while(results.Count > 0)
				{
					...
					results = Top(batchSize, (DirectMessageColumns)where(columns) && orderBy.Column > top, orderBy, database);
				}
			});			
		}
