...
namespace Bam.Net.Translation
{
	...
	public partial class Language: Bam.Net.Data.Dao
	{
	...
	public OtherNameCollection OtherNamesByLanguageId
	{
		...
		/// </summary>			 
		[Bam.Net.Exclude]
		public static async Task BatchQuery<ColType>(int batchSize, QueryFilter filter, Action<IEnumerable<Language>> batchProcessor, Bam.Net.Data.OrderBy<LanguageColumns> orderBy, Database database = null)
		{
		...
		public static async Task BatchQuery<ColType>(int batchSize, WhereDelegate<LanguageColumns> where, Action<IEnumerable<Language>> batchProcessor, Bam.Net.Data.OrderBy<LanguageColumns> orderBy, Database database = null)
		{
			await System.Threading.Tasks.Task.Run(async ()=>
			{
				...
				while(results.Count > 0)
				{
					...
					results = Top(batchSize, (LanguageColumns)where(columns) && orderBy.Column > top, orderBy, database);
				}
			});			
		}
