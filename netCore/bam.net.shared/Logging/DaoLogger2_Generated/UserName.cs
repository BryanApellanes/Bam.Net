...
namespace Bam.Net.Logging.Data
{
	...
	public partial class UserName: Bam.Net.Data.Dao
	{
		...
		private void SetChildren()
		{
			...
			if(_database != null)
			{
				this.ChildCollections.Add("Event_UserNameId", new EventCollection(Database.GetQuery<EventColumns, Event>((c) => c.UserNameId == GetULongValue("Id")), this, "UserNameId"));				
			}						
		}
	...
	public EventCollection EventsByUserNameId
	{
		...
		public static UserNameCollection LoadAll(Database database = null)
		{
			...
			sql.Select<UserName>();
			var results = new UserNameCollection(db, sql.GetDataTable(db))
			{
				Database = db
			};
			return results;
		...
		/// </summary>			 
		[Bam.Net.Exclude]
		public static async Task BatchQuery<ColType>(int batchSize, QueryFilter filter, Action<IEnumerable<UserName>> batchProcessor, Bam.Net.Data.OrderBy<UserNameColumns> orderBy, Database database = null)
		{
		...
		/// </summary>	
		[Bam.Net.Exclude]
		public static async Task BatchQuery<ColType>(int batchSize, WhereDelegate<UserNameColumns> where, Action<IEnumerable<UserName>> batchProcessor, Bam.Net.Data.OrderBy<UserNameColumns> orderBy, Database database = null)
		{
			await System.Threading.Tasks.Task.Run(async ()=>
			{
				UserNameColumns columns = new UserNameColumns();
				...
				while(results.Count > 0)
				{
					...
					results = Top(batchSize, (UserNameColumns)where(columns) && orderBy.Column > top, orderBy, database);
				}
			});			
		}

		public static UserName GetById(uint id, Database database = null)
		{
		...
		public static UserName GetById(ulong id, Database database = null)
		{
			return OneWhere(c => c.KeyColumn == id, database);
		}
		...
		[Bam.Net.Exclude]
		public static UserNameCollection Top(int count, QueryFilter where, string orderBy = null, SortOrder sortOrder = SortOrder.Ascending, Database database = null)
		{
			Database db = database ?? Db.For<UserName>();
			QuerySet query = GetQuerySet(db);
			query.Top<UserName>(count);
