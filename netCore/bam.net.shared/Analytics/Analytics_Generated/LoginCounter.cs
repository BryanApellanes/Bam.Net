...
namespace Bam.Net.Analytics
{
	...
	public partial class LoginCounter: Bam.Net.Data.Dao
	{
	...
	public UserIdentifier UserIdentifierOfUserIdentifierId
	{
		...
		/// </summary>			 
		[Bam.Net.Exclude]
		public static async Task BatchQuery<ColType>(int batchSize, QueryFilter filter, Action<IEnumerable<LoginCounter>> batchProcessor, Bam.Net.Data.OrderBy<LoginCounterColumns> orderBy, Database database = null)
		{
		...
		public static async Task BatchQuery<ColType>(int batchSize, WhereDelegate<LoginCounterColumns> where, Action<IEnumerable<LoginCounter>> batchProcessor, Bam.Net.Data.OrderBy<LoginCounterColumns> orderBy, Database database = null)
		{
			await System.Threading.Tasks.Task.Run(async ()=>
			{
				...
				while(results.Count > 0)
				{
					...
					results = Top(batchSize, (LoginCounterColumns)where(columns) && orderBy.Column > top, orderBy, database);
				}
			});			
		}
