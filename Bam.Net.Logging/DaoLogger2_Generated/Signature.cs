...
namespace Bam.Net.Logging.Data
{
	...
	public EventCollection EventsBySignatureId
	{
		...
		public static async Task BatchAll(int batchSize, Func<SignatureCollection, Task> batchProcessor, Database database = null)
		{
			await Task.Run(async ()=>
			{
				SignatureColumns columns = new SignatureColumns();
				var orderBy = Order.By<SignatureColumns>(c => c.KeyColumn, SortOrder.Ascending);
		...
		public static async Task BatchQuery(int batchSize, QueryFilter filter, Func<SignatureCollection, Task> batchProcessor, Database database = null)
		{
			await BatchQuery(batchSize, (c) => filter, batchProcessor, database);			
		}

		public static async Task BatchQuery(int batchSize, WhereDelegate<SignatureColumns> where, Func<SignatureCollection, Task> batchProcessor, Database database = null)
		{
			await Task.Run(async ()=>
			{
				SignatureColumns columns = new SignatureColumns();
