...
namespace Bam.Net.Shop
{
	...
	[Serializable]
	[Bam.Net.Data.Table("ShopPromotion", "Shop")]
	public partial class ShopPromotion: Dao
	{
		...
		public ShopPromotion(DataRow data)
			: base(data)
		{
			this.SetKeyColumnName();
			this.SetChildren();
		...
		public ShopPromotion(Database db)
			: base(db)
		{
			this.SetKeyColumnName();
			this.SetChildren();
		...
		public ShopPromotion(Database db, DataRow data)
			: base(db, data)
		{
			this.SetKeyColumnName();
			this.SetChildren();
	...
	public Promotion PromotionOfPromotionId
	{
		...
		[Bam.Net.Exclude]
		public static async Task BatchAll(int batchSize, Action<IEnumerable<ShopPromotion>> batchProcessor, Database database = null)
		{
			await Task.Run(async ()=>
			{
				ShopPromotionColumns columns = new ShopPromotionColumns();
				var orderBy = Bam.Net.Data.Order.By<ShopPromotionColumns>(c => c.KeyColumn, Bam.Net.Data.SortOrder.Ascending);
		...
		[Bam.Net.Exclude]
		public static async Task BatchQuery(int batchSize, QueryFilter filter, Action<IEnumerable<ShopPromotion>> batchProcessor, Database database = null)
		{
			await BatchQuery(batchSize, (c) => filter, batchProcessor, database);			
		}

		[Bam.Net.Exclude]
		public static async Task BatchQuery(int batchSize, WhereDelegate<ShopPromotionColumns> where, Action<IEnumerable<ShopPromotion>> batchProcessor, Database database = null)
		{
			await Task.Run(async ()=>
			{
				ShopPromotionColumns columns = new ShopPromotionColumns();
				...
				while(results.Count > 0)
				{
					...
					long topId = results.Select(d => d.Property<long>(columns.KeyColumn.ToString())).ToArray().Largest();
					results = Top(batchSize, (ShopPromotionColumns)where(columns) && columns.KeyColumn > topId, orderBy, database);
				}
			});			
		}
		...
		public static ShopPromotion GetByUuid(string uuid, Database database = null)
		{
			return OneWhere(c => Bam.Net.Data.Query.Where("Uuid") == uuid, database);
		}

		public static ShopPromotion GetByCuid(string cuid, Database database = null)
		{
		...
		[Bam.Net.Exclude]
		public static ShopPromotionCollection Top(int count, QueryFilter where, Database database)
		{
			return Top(count, where, null, database);
		}
		/// <summary>
		...
		/// <summary>
		/// Return the count of ShopPromotions
		/// </summary>
