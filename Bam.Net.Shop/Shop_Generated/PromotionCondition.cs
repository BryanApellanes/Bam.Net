...
namespace Bam.Net.Shop
{
	...
	[Serializable]
	[Bam.Net.Data.Table("PromotionCondition", "Shop")]
	public partial class PromotionCondition: Dao
	{
		...
		public PromotionCondition(DataRow data)
			: base(data)
		{
			this.SetKeyColumnName();
			this.SetChildren();
		...
		public PromotionCondition(Database db)
			: base(db)
		{
			this.SetKeyColumnName();
			this.SetChildren();
		...
		public PromotionCondition(Database db, DataRow data)
			: base(db, data)
		{
			this.SetKeyColumnName();
			this.SetChildren();
	...
	public Promotion PromotionOfPromotionId
	{
		...
		/// </summary>
		[Bam.Net.Exclude]
		public static async Task BatchAll(int batchSize, Action<IEnumerable<PromotionCondition>> batchProcessor, Database database = null)
		{
			await Task.Run(async ()=>
			{
				PromotionConditionColumns columns = new PromotionConditionColumns();
				var orderBy = Bam.Net.Data.Order.By<PromotionConditionColumns>(c => c.KeyColumn, Bam.Net.Data.SortOrder.Ascending);
		...
		/// </summary>			 
		[Bam.Net.Exclude]
		public static async Task BatchQuery(int batchSize, QueryFilter filter, Action<IEnumerable<PromotionCondition>> batchProcessor, Database database = null)
		{
		...
		/// </summary>	
		[Bam.Net.Exclude]
		public static async Task BatchQuery(int batchSize, WhereDelegate<PromotionConditionColumns> where, Action<IEnumerable<PromotionCondition>> batchProcessor, Database database = null)
		{
			await Task.Run(async ()=>
			{
				PromotionConditionColumns columns = new PromotionConditionColumns();
				...
				while(results.Count > 0)
				{
					...
					long topId = results.Select(d => d.Property<long>(columns.KeyColumn.ToString())).ToArray().Largest();
					results = Top(batchSize, (PromotionConditionColumns)where(columns) && columns.KeyColumn > topId, orderBy, database);
				}
			});			
		}
		...
		public static PromotionCondition GetByUuid(string uuid, Database database = null)
		{
			return OneWhere(c => Bam.Net.Data.Query.Where("Uuid") == uuid, database);
		}

		public static PromotionCondition GetByCuid(string cuid, Database database = null)
		{
		...
		[Bam.Net.Exclude]
		public static PromotionConditionCollection Top(int count, QueryFilter where, Database database)
		{
			return Top(count, where, null, database);
		}
		/// <summary>
		...
		/// <summary>
		/// Return the count of PromotionConditions
		/// </summary>
