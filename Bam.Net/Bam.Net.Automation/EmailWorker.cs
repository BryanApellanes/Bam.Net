...
namespace Bam.Net.Automation
{
    public class EmailWorker: NotificationWorker
    {
        ...
        public string Password
        {
            get
            {
                return Orchestrator.Default.SecureGet("{0}_{1}"._Format(typeof(EmailWorker).Name, Name));
            }
            set
            {
                Orchestrator.Default.SecureSet("{0}_{1}"._Format(typeof(EmailWorker).Name, Name), value);
            }
        }
